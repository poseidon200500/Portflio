...
void dec_to_big(s21_decimal *dst, s21_big_decimal *big_dst) {
...
s21_big_decimal *init_big_decimal() {
...
int big_to_dec(s21_big_decimal *big_dst, s21_decimal *dst) {
...
void null_big_dec(s21_big_decimal *value) {
  for (int i = 0; i < 8; ++i) value->bits[i] = 0;
}

unsigned get_sign_big(s21_big_decimal *value) {
...
//ошибки
int set_scale_big(s21_big_decimal *dec, int scale_value) {
...
// void print_dec_big(s21_big_decimal val) {
...
void great_scale(s21_big_decimal *val1, int scale2) {
  s21_big_decimal *temp1 = init_big_decimal(), *temp2 = init_big_decimal();
...
void normalization_big(s21_big_decimal *val1, s21_big_decimal *val2) {
...
void copy_big_decimal(s21_big_decimal *sourse, s21_big_decimal *copy) {
...
// 1 - yes, 0 - no
int good_big(s21_big_decimal a) {
  return (a.bits[3] > 0 || a.bits[4] > 0 || a.bits[5] > 0 || a.bits[6] > 0) ? 0
                                                                            : 1;
}

// 1 - yes, 0 - no
int is_zero_big(s21_big_decimal *a) {
  int k = 1;
  for (int i = 0; i < 7; i++)
    if (a->bits[i] > 0) k = 0;
  return k;
...
int overflow(s21_big_decimal *big_dec, s21_decimal *dec) {
  ...
  if (big_dec->bits[3] == 0 && big_dec->bits[4] == 0 && big_dec->bits[5] == 0 &&
    ...
    s21_big_decimal temp = {0}, trunc = {0}, big_result = {0},
                    ten = {{10, 0, 0, 0}};
    ...
    while (scale != 0) {
      ...
      if (trunc.bits[3] == 0 && trunc.bits[4] == 0 && trunc.bits[5] == 0 &&
        ...
        s21_big_decimal mode, trunc_temp;
        null_big_dec(&mode);
...
void overflow_help2(s21_big_decimal *mode, int sign, s21_big_decimal *trunc,
                    s21_big_decimal *big_result) {
  set_sign_big(mode, 0);
  s21_big_decimal check_mode = {{5, 0, 0, 0}}, number = {{1, 0, 0, 0}};
