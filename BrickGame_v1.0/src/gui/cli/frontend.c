...
void init_colors() {
  ...
  init_color(O_FIGURE, 0, 1000, 1000);
  init_color(I_FIGURE, 1000, 1000, 0);
  init_color(S_FIGURE, 500, 0, 500);
  init_color(Z_FIGURE, 0, 1000, 0);
  init_color(L_FIGURE, 1000, 0, 0);
  init_color(J_FIGURE, 0, 0, 1000);
  init_color(T_FIGURE, 1000, 500, 0);

  init_pair(O_FIGURE, O_FIGURE, O_FIGURE);
  init_pair(I_FIGURE, I_FIGURE, I_FIGURE);
  init_pair(S_FIGURE, S_FIGURE, S_FIGURE);
  init_pair(Z_FIGURE, Z_FIGURE, Z_FIGURE);
  init_pair(L_FIGURE, L_FIGURE, L_FIGURE);
  init_pair(J_FIGURE, J_FIGURE, J_FIGURE);
  init_pair(T_FIGURE, T_FIGURE, T_FIGURE);
}
...
WINDOW *print_game_field(GameInfo_t gi) {
  ...
  for (int i = 0; i < FIELD_N; i++) {
    for (int j = 0; j < FIELD_M; j++) {
      if (gi.field[i][j] == 1) {
        wattron(game_window, COLOR_PAIR(gi.color_field[i][j]));
        mvwprintw(game_window, i + 1, 3 * j + 1, "   ");
        wattroff(game_window, COLOR_PAIR(gi.color_field[i][j]));
      } else {
...
void print_next_figure(WINDOW *info_window, GameInfo_t gi) {
  ...
  for (int i = 0; i < gi.next_size; i++) {
    for (int j = 0; j < gi.next_size; j++) {
      if (gi.next[i][j] == 1) {
        wattron(info_window, COLOR_PAIR(gi.next_color));
        mvwprintw(info_window, i + 4, j * 3 + 4, "   ");
        wattroff(info_window, COLOR_PAIR(gi.next_color));
      }
    }
  }
}