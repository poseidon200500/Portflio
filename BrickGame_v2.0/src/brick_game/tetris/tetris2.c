...
void init_game() {
  ...
  gs->color_field = create_matrix(FIELD_N, FIELD_M);

  srand(get_time());

  int size, color;
  gs->next_figure = generate_figure(&size, &color);
  gs->next_figure_size = size;
  gs->next_figure_color = color;

  gs->figure = create_matrix(size, size);
  gs->figure_size = size;
  gs->figure_color = color;
...
int **generate_figure(int *size, int *color) {
  ...
  *size = (random_type == 0) ? 4 : (random_type == 3) ? 2 : 3;
  ...
  if (figure != NULL) fill_figure(random_type, figure, color);

  return figure;
}

void fill_figure(int random_type, int **figure, int *color) {
  switch (random_type) {
    case 0:
      figure[0][0] = 1;
      figure[0][1] = 1;
...
GameInfo_t updateCurrentState(GameState_t *gs) {
  ...
  gi.color_field = create_matrix(FIELD_N, FIELD_M);
  copy_matrix(FIELD_N, FIELD_M, gs->field, gi.field);
  copy_matrix(FIELD_N, FIELD_M, gs->color_field, gi.color_field);

  if (gi.field != NULL) update_field(gs, gi.field, gi.color_field);
  ...
  gi.next_size = size;
  gi.next_color = gs->next_figure_color;

  gi.high_score = gs->high_score;
  gi.pause = gs->pause;

  gi.figure.array = create_matrix(gs->figure_size,gs->figure_size);
  copy_matrix(gs->figure_size,gs->figure_size,gs->figure,gi.figure.array);
  gi.figure.x = gs->x;
  gi.figure.y = gs->y;

  gi.next_figure.array = create_matrix(gs->next_figure_size,gs->next_figure_size);
  copy_matrix(gs->next_figure_size,gs->next_figure_size,gs->next_figure,gi.next_figure.array);
  gi.next_figure.x = gs->x;
  gi.next_figure.y = gs->y;
  ...
  for (int i = 0; i < gs->figure_size; i++) {
    for (int j = 0; j < gs->figure_size; j++) {
      ...
      if (gs->figure[i][j] == 1 && y > -1 && y < FIELD_N && x > -1 &&
          x < FIELD_M) {
        field[y][x] = 1;
        color_field[y][x] = gs->figure_color;
      }
    }
  }
}
...
void finish_game(GameState_t *gs) {
  ...
  free_matrix(FIELD_N, gs->color_field);
  ...
  for (int i = 0; i < gs->figure_size && !is_game_over; i++) {
    for (int j = 0; j < gs->figure_size; j++) {
      if (gs->figure[i][j] == 1) {
        ...
        gs->color_field[gs->y + i][gs->x + j] = gs->figure_color;
      }
    }
  }
...
void free_game_info(GameInfo_t *gi) {
  ...
  free_matrix(FIELD_N, gi->color_field);
